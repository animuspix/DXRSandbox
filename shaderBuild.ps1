...
for ($dirCtr = 0; $dirCtr -lt $dirs.Count; $dirCtr++)
{
	...
	for ($i = 0; $i -lt $files.Count; $i++)
	{
		...
		if ($fname.Contains(".hlsl") -and -not $fname.Contains(".hlsli"))
		{
			...
			if ($buildingCompute)
			{
				thirdparty\\dxc_2025_07_14\\bin\\x64\\dxc.exe -E main -T cs_6_6 -Zi -Fd "Shaders\PDBs\" -Fo $outPath $fFullName
			...
			elseif ($buildingGraphics)
			{
				...
				thirdparty\\dxc_2025_07_14\\bin\\x64\\dxc.exe -E main_ps -T ps_6_6 -Fo $psoPath $fFullName
				$success = $?

				$vsoPath = $outDir + $extensions[$dirCtr][1]
				thirdparty\\dxc_2025_07_14\\bin\\x64\\dxc.exe -E main_vs -T vs_6_6 -Fo $vsoPath $fFullName
				$success = $success -and $?
			}
