...
public:
	...
	// Get indices for all the resources bound to this pipeline
	// (only valid after ResolveBindings)
	const uint32_t GetBindingIndex(PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> handle) const;
	...
	struct ResourcePermSetSelector
	{
		using permissionType = std::conditional_t<variant == ResourceViews::IBUFFER ||
			variant == ResourceViews::VBUFFER ||
			variant == ResourceViews::STRUCTBUFFER_RW ||
			variant == ResourceViews::RT_ACCEL_STRUCTURE ||
			variant == ResourceViews::CBUFFER,
			GPUResrcPermSetGeneric,
			GPUResrcPermSetTextures>;
	};

	template<ResourceViews variant>
	struct ResourcePermissionsSelector
	{
		using permissionType = std::conditional_t<variant == ResourceViews::IBUFFER ||
			variant == ResourceViews::VBUFFER ||
			variant == ResourceViews::STRUCTBUFFER_RW ||
			variant == ResourceViews::RT_ACCEL_STRUCTURE ||
			variant == ResourceViews::CBUFFER,
			GPU_RESRC_ACCESS_PERMISSIONS_GENERIC,
			GPU_RESRC_ACCESS_PERMISSIONS_TEXTURES>;
	};

	// Generic/overloaded alternative calls
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterResource(GPUResource<ResourceViews::CBUFFER>::resrc_desc desc, GPUResrcPermSetGeneric accessSettings) { return RegisterCBuffer(desc, accessSettings); }
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterResource(GPUResource<ResourceViews::STRUCTBUFFER_RW>::resrc_desc desc, GPUResrcPermSetGeneric accessSettings) { return RegisterStructBuffer(desc, accessSettings); }
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterResource(GPUResource<ResourceViews::TEXTURE_DIRECT_WRITE>::resrc_desc desc, GPUResrcPermSetTextures accessSettings) { return RegisterTextureDirectWrite(desc, accessSettings); }
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterResource(GPUResource<ResourceViews::TEXTURE_SUPPORTS_SAMPLING>::resrc_desc desc, GPUResrcPermSetTextures accessSettings) { return RegisterTextureSampleable(desc, accessSettings); }
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterResource(GPUResource<ResourceViews::TEXTURE_RENDER_TARGET>::resrc_desc desc, GPUResrcPermSetTextures accessSettings) { return RegisterRenderTarget(desc, accessSettings); }
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterResource(GPUResource<ResourceViews::TEXTURE_DEPTH_STENCIL>::resrc_desc desc, GPUResrcPermSetTextures accessSettings) { return RegisterDepthStencil(desc, accessSettings); }
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterResource(GPUResource<ResourceViews::VBUFFER>::resrc_desc desc, GPUResrcPermSetGeneric accessSettings) { return RegisterVBuffer(desc, accessSettings); }
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterResource(GPUResource<ResourceViews::IBUFFER>::resrc_desc desc, GPUResrcPermSetGeneric accessSettings) { return RegisterIBuffer(desc, accessSettings); }
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterResource(GPUResource<ResourceViews::RT_ACCEL_STRUCTURE>::resrc_desc desc, GPUResrcPermSetGeneric accessSettings) { return RegisterAccelerationStructure(desc, accessSettings); }

	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterResource(GPUResource<ResourceViews::STRUCTBUFFER_RW>::resrc_desc desc, GPU_RESRC_ACCESS_PERMISSIONS_GENERIC accessSettings) { return RegisterStructBuffer(desc, accessSettings); }
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterResource(GPUResource<ResourceViews::TEXTURE_DIRECT_WRITE>::resrc_desc desc, GPU_RESRC_ACCESS_PERMISSIONS_TEXTURES accessSettings) { return RegisterTextureDirectWrite(desc, accessSettings); }
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterResource(GPUResource<ResourceViews::TEXTURE_SUPPORTS_SAMPLING>::resrc_desc desc, GPU_RESRC_ACCESS_PERMISSIONS_TEXTURES accessSettings) { return RegisterTextureSampleable(desc, accessSettings); }
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterResource(GPUResource<ResourceViews::TEXTURE_RENDER_TARGET>::resrc_desc desc, GPU_RESRC_ACCESS_PERMISSIONS_TEXTURES accessSettings) { return RegisterRenderTarget(desc, accessSettings); }
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterResource(GPUResource<ResourceViews::TEXTURE_DEPTH_STENCIL>::resrc_desc desc, GPU_RESRC_ACCESS_PERMISSIONS_TEXTURES accessSettings) { return RegisterDepthStencil(desc, accessSettings); }
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterResource(GPUResource<ResourceViews::VBUFFER>::resrc_desc desc, GPU_RESRC_ACCESS_PERMISSIONS_GENERIC accessSettings) { return RegisterVBuffer(desc, accessSettings); }
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterResource(GPUResource<ResourceViews::IBUFFER>::resrc_desc desc, GPU_RESRC_ACCESS_PERMISSIONS_GENERIC accessSettings) { return RegisterIBuffer(desc, accessSettings); }
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterResource(GPUResource<ResourceViews::RT_ACCEL_STRUCTURE>::resrc_desc desc, GPU_RESRC_ACCESS_PERMISSIONS_GENERIC accessSettings) { return RegisterAccelerationStructure(desc, accessSettings); }

	// Resource sharing/transition interfaces
	...
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> RegisterSharedResource(PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC> handle, ResourceViews view)
	{
		switch (view)
		{
		case ResourceViews::STRUCTBUFFER_RW:
			return RegisterStructBuffer(handle);
		case ResourceViews::TEXTURE_DIRECT_WRITE:
			return RegisterTextureDirectWrite(handle);
		case ResourceViews::TEXTURE_SUPPORTS_SAMPLING:
			return RegisterTextureSampleable(handle);
		case ResourceViews::TEXTURE_RENDER_TARGET:
			return RegisterRenderTarget(handle);
		case ResourceViews::TEXTURE_DEPTH_STENCIL:
			//pipes[stage].RegisterDepthStencil(handle);
			OutputDebugStringA("Depth stencils cannot currently be reused across passes/stages");
			assert(false);// Should implement using message boxes and the windows i/o pump
			return PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC>();
		case ResourceViews::VBUFFER:
			return RegisterVBuffer(handle);
		case ResourceViews::IBUFFER:
			return RegisterIBuffer(handle);
		case ResourceViews::RT_ACCEL_STRUCTURE:
			//return RegisterAccelerationStructure(handle);
			return PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC>();
		default:
			return PipelineObjectHandle<PIPELINE_OBJ_TYPES::RESRC>();
			assert(false); // Unsupported view! Note ResourceViews::NUM_VARIANTS is a counting element, not a resource view itself (really need to implement messageboxes for errors like this)
			break;
		}
	}
	...
	PipelineObjectHandle<PIPELINE_OBJ_TYPES::SHADER> RegisterGraphicsShader(const char* dxilPathVertex, const char* dxilPathPixel, const RasterSettings& gfxSettings);
	...
	void BakeCmdList(uint32_t* rootConstants, uint32_t numConstants); // Bakes our scheduler down to an API command-list for submission (through ExecuteCommandLists() on DX12)
	void SubmitCmdList(bool synchronous, uint32_t* rootConstants, uint32_t numConstants); // "synchronous" indicates whether the API should wait after this cmd-list is submitted, in case a later pipeline depends on some of the work it performed
...
private:
	...
	struct PipelineEvent
	{
		...
		void IssueToCmdList(DXWrapper::DataHandle<D3D_CMD_LIST> clientCmdList, DXWrapper::DataHandle<D3D_ROOTSIG> rootSig,
			uint32_t pipelineID, uint32_t* rootConstants, uint32_t numRootConstants); // Defined separately to reduce header leakages
