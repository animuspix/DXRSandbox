...
public:
	...
	struct ResourceBindList
	{
		...
		// Order needs to be specified for root-signature resources (besides cbuffers), since they're accessed
		// explicitly in shaders

		enum DYNAMICALLY_ORDERED_BINDINGS
		{
			STRUCTBUFFER,
			TEXTURE_SAMPLED,
			TEXTURE_RW
		...
		DYNAMICALLY_ORDERED_BINDINGS dynamicResourceBindingOrder[XPlatConstants::maxResourcesPerPipeline];
	...
	{
		uint64_t orderedUntypedResourceHandles[XPlatConstants::maxResourcesPerPipeline] = {};
		uint32_t numResources;

		DataHandle<D3D_ROOTSIG> rootsig;
	};

	static BindingMetadata ResolveRootSignature(ResourceBindList bindings, bool mayUseGraphics, uint32_t pipelineID);
...
public:
	...
	static void BindComputeResources(DataHandle<D3D_CMD_LIST> pipe_work, DataHandle<D3D_ROOTSIG> rootSig, uint8_t pipelineID, uint32_t* rootConstants, uint32_t numRootConstants);
	static void BindGFX_Resources(DataHandle<D3D_CMD_LIST> pipe_work, DataHandle<D3D_ROOTSIG> rootSig, uint8_t pipelineID, uint32_t* rootConstants, uint32_t numRootConstants);
