...
struct GPUResource
{
	...
	public:
		...
		const size_t footprint() const
		{
			uint32_t footprint = 0;
			for (uint32_t i = 0; i < numDimensions; i++)
			{
				footprint += desc.stride * desc.dimensions[i];
			}
	...
	struct resrc_desc_custom_fmt : public resrc_descs_shared
	{
		...
		{
			this->stride = sizeof(bufferType);
			this->dimensions[0] = 1;

			this->srcData = _srcData.GetByteSpan();
			this->resrcName = name;
		}
		...
		{
			this->stride = sizeof(v);
			this->dimensions[0] = numElts;
		...
		{
			this->stride = eltStride;
			this->dimensions[0] = numElts;
			this->srcData = _srcData;
			this->resrcName = name;
		}
	};
	...
	static constexpr bool texture = variant == ResourceViews::TEXTURE_SUPPORTS_SAMPLING || variant == ResourceViews::TEXTURE_DIRECT_WRITE ||
		variant == ResourceViews::TEXTURE_RENDER_TARGET || variant == ResourceViews::TEXTURE_DEPTH_STENCIL;
...
private:
	...
	struct StdTextureGenerator
	{
		void operator()(resrc_desc_texture_fmt desc, DXWrapper::DataHandle<D3D_TEXTURE>* texture_out, AccessPermissions accessSettings, uint32_t pipelineID)
		{
			...
			const TextureViews textureView = variant == ResourceViews::TEXTURE_DIRECT_WRITE ? TextureViews::DIRECT_WRITE :
				variant == ResourceViews::TEXTURE_SUPPORTS_SAMPLING ? TextureViews::SUPPORTS_SAMPLING :
				variant == ResourceViews::TEXTURE_RENDER_TARGET ? TextureViews::RENDER_TARGET :
				TextureViews::DEPTH_STENCIL;

			*texture_out = DXWrapper::GenerateStandardTexture(desc.dimensions[0], desc.dimensions[1], desc.fmt, desc.msaa, accessSettings, textureView, desc.srcData, pipelineID);
...
public:
	...
	{
		// Is the source resource shareable?
		...
		if (variant == ResourceViews::IBUFFER || variant == ResourceViews::RT_ACCEL_STRUCTURE || variant == ResourceViews::CBUFFER || variant == ResourceViews::VBUFFER || variant == ResourceViews::STRUCTBUFFER_RW)
		{
